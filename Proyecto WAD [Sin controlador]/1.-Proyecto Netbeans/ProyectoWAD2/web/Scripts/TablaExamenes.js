...
class Index extends React.Component {
  ...
  eliminar = value => () =>{
    ...
    if (confirm('¿Estás seguro de borrar este examen?')) {
       window.location.replace("BorrarExamenAction?examenEliminado="+value)
    } 
  } 
  ...
  render() {
    ...
    <Paper className={classes.paper} >
        ...
        <br /><br />
        <a href="EscribirExamenAuxAction" >
          ...
          {this.state.examenes.map(row => {
            return (
              <TableRow className={classes.row}>
                ...
                <CustomTableCell>
                    <IconButton aria-label="Evaluar Examen" className={classes.margin} onClick = {() => window.location.replace("EvaluarPreguntaAction?err=0&examen="+row)}>
                        <Icon className={classes.icon}>visibility</Icon>
                    </IconButton>
                    <IconButton aria-label="Editar Examen" className={classes.margin} onClick = {() => window.location.replace("EditarExamenAction?examen="+row)}>
