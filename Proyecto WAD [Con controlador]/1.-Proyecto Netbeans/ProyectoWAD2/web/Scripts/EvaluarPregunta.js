    ...
    return (
     <Grid item xs={12}>
      <Paper className={classes.paper}>    
        ...
        >
          <BottomNavigationAction label={'Anterior: "'+this.state.anterior+'"'}  onClick = {() => this.enviarPost("EvaluarPreguntaAction?examen="+this.state.examen+"&pregunta="+this.state.anterior+"&err="+this.state.pregIncorrectas)} icon={ <Icon className={classes.icon}>skip_previous</Icon>  } />
          <BottomNavigationAction label={'Siguiente: "'+this.state.siguiente+'"'} onClick = {() => {
              if(this.state.siguiente=="Evaluacion del Examen")
                this.enviarPost("ControladorPrincipal?accion=EvaluarExamenAction&examen="+this.state.examen+"&pregunta="+this.state.siguiente+"&err="+this.evaluar()) 
              else
                this.enviarPost("ControladorPrincipal?accion=EvaluarPreguntaAction&examen="+this.state.examen+"&pregunta="+this.state.siguiente+"&err="+this.evaluar()) 
