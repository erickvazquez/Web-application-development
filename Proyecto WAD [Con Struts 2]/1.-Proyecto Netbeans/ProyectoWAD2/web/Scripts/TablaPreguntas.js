  ...
  leerjson(){
   ...
   eliminar = value => () =>{
    ...
    if (confirm('¿Estás seguro de borrar esta pregunta?')) {
       window.location.replace("BorrarPreguntaAction2?preguntaEliminada="+value)
    } 
  } 
          ...
          {this.state.preguntas.map(row => {
            return (
              <TableRow className={classes.row}>
                ...
                <CustomTableCell> 
                    <IconButton aria-label="Ver Pregunta" className={classes.margin} onClick = {() => window.location.replace("VerPreguntaAction2?pregunta="+row)}> 
                        <Icon className={classes.icon}>visibility</Icon> 
                    </IconButton>  
                    <IconButton aria-label="Editar Pregunta" className={classes.margin} onClick = {() => window.location.replace("EditarPreguntaAction2?pregunta="+row)}>  
